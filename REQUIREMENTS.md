...
2. 「AIで整理」を押し、タイトル、内容、担当者、期限と、必要な場合は実行手順のサブタスク候補を得る。
...
| F-05 | 候補確認・修正 | 登録前に全基本項目とサブタスクを編集できる |
...
| W-02 | 自由列マッピング | 任意の複数列をタイトル、内容、担当者、期限、識別キー、親キー、階層レベル、階層列へ割り当てられる |
...
| W-05 | プレビュー | Asanaへ送信する前に、変換後の親子関係、担当者文字列、日付、除外行、検証エラーを表で確認し、タイトル・担当者・期限・対象行を修正できる |
| W-06 | 一括登録 | 利用者の最終確認後だけ登録し、行単位の成功・失敗・Asana GIDを保存する |
...
- WBSの開始日、依存関係、タグ、custom field、既存Asanaタスクの更新・削除
