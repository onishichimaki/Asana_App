...
Excel/CSVから複数のタスクを一括変換する機能は実現可能であり、現在の1件登録フローとは分離したPhase 2として扱う。

| ID | 要件 | 受入条件 |
|---|---|---|
| W-01 | ファイル読込 | `.xlsx` と `.csv` をブラウザーで読み、sheet・header行・data開始行を選択できる |
| W-02 | 自由列マッピング | 任意の列をタイトル、内容、担当者、開始日、期限、親ID、階層、project、section、custom fieldへ割り当てられる |
| W-03 | 階層指定 | 「親ID列」または「階層レベル列」を利用者が選び、親タスクとサブタスクへ変換できる |
| W-04 | 変換テンプレート | 列マッピング、日付形式、階層ルール、既定project/sectionをプロジェクト別の名前付きテンプレートとして保存・再利用できる |
| W-05 | プレビュー | Asanaへ送信する前に、変換後の親子関係、担当者解決、日付、除外行、検証エラーを表で確認・修正できる |
...
- Excel/CSVのWBS一括登録は現MVPには含めず、上記Phase 2要件として実装する
