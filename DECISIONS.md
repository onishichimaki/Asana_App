...
## D-042: 認証許可前の説明はクライアント内画面確認モードで行う

- 日付: 2026-07-26
- 状態: 採用・実装済み
- 判断: ログイン画面に「ログインせず画面を見る」を設け、通常入力・見本整理・Excel/CSV読取・列マッピング・親子プレビューを未認証で操作可能にする。見本project/sectionを明示し、業務API、SQL保存、AI API、Asana情報取得・登録、履歴、利用者設定は呼び出さない。APIの認証fallback policyとAsana接続要件は変更しない。
- 理由: Asana OAuthアプリや社内インフラの許可申請には先に実画面を説明する必要がある一方、説明目的で未認証のデータアクセスや登録権限を開放すべきではないため。

## D-043: 会社への移管は固定Releaseのソーススナップショットから行う

- 日付: 2026-07-26
- 状態: 採用・文書化済み
- 判断: 個人GitHubから会社環境への初回移管は、版とSHA-256を固定したソースZIPを会社承認済み経路で取得し、検査後に会社の非公開GitHubへ新規登録する。会社リポジトリから個人リポジトリを継続参照せず、会社環境で依存関係取得、ビルド、テスト、配布物生成をやり直す。
- 理由: 個人GitHubと会社本番の継続的な信頼関係を作らず、持込み元、版、検査結果を説明可能にし、個人PCの生成物や秘密情報が会社本番へ混入するリスクを下げるため。
