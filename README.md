...
- 認証許可前でも安全に説明できる、保存・送信なしの画面確認モード
...
ログイン画面の「ログインせず画面を見る」を押すと、Asana OAuthの社内許可前でも次の操作を説明・確認できます。

- 1件ずつ登録する入力画面と、ローカルの見本整理・候補編集
- テキスト、貼り付け、議事録、画像OCR、ブラウザーの音声入力
- Excel/CSVのブラウザー内読取、自動判定、列マッピング、親子プレビュー
- 見本のAsanaプロジェクト・セクション選択

画面確認モードでは入力内容を本アプリのAPIへ送らず、SQL Serverへの保存、AI API呼出し、Asana情報取得・登録、履歴・利用者設定を行いません。「見学終了」または「ログインへ戻る」でログイン画面へ戻ります。音声入力そのものは利用ブラウザーのWeb Speech API仕様に従います。
